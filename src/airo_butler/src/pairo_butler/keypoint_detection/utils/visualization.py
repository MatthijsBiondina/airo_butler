...
from pairo_butler.keypoint_detection.utils.heatmap import generate_channel_heatmap
...
def get_logging_label_from_channel_configuration(
    channel_configuration: List[List[str]], mode: str
) -> str:
...
def overlay_images_with_keypoints(
    ...
    for channel_idx, channel_keypoints in enumerate(image_keypoints):
        ...
        draw.text(
            ...
            get_logging_label_from_channel_configuration(
                channel_configuration[channel_idx], ""
            ),
            fill=color_pool[channel_idx],
...
def visualize_predicted_keypoints(
    ...
    drawn_images = torch.stack(
        [
            torch.from_numpy(np.array(image)).permute(2, 0, 1) / 255

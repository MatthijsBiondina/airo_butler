...
from pairo_butler.keypoint_detection.data.coco_parser import (
    CocoImage,
    CocoKeypointCategory,
    CocoKeypoints,
)
from pairo_butler.keypoint_detection.data.imageloader import ImageDataset, ImageLoader
from pairo_butler.keypoint_detection.types import COCO_KEYPOINT_TYPE, IMG_KEYPOINTS_TYPE
...
class COCOKeypointsDataset(ImageDataset):
    ...
    def __getitem__(self, index) -> Tuple[torch.Tensor, IMG_KEYPOINTS_TYPE]:
        ...
        keypoints = [
            [
                [math.floor(keypoint[0]), math.floor(keypoint[1])]
                for keypoint in channel_keypoints
            ]

...
from pairo_butler.keypoint_detection.models.metrics import (
    DetectedKeypoint,
    Keypoint,
    KeypointAPMetrics,
)
from pairo_butler.keypoint_detection.utils.heatmap import (
    BCE_loss,
    create_heatmap_batch,
    get_keypoints_from_heatmap_batch_maxpool,
)
from pairo_butler.keypoint_detection.utils.visualization import (
...
class KeypointDetector(pl.LightningModule):
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        should_log_ap = (
            self.is_ap_epoch() and batch_idx < 20
        )  # limit AP calculation to first 20 batches to save time
    ...
    def log_channel_predictions_grids(self, image_grids, mode: str):
        for channel_configuration, grid in zip(
            ...
            label = get_logging_label_from_channel_configuration(
                channel_configuration, mode
            )
    ...
    def visualize_predicted_keypoints(self, result_dict):
        ...
        grid = visualize_predicted_keypoints(
            images, predicted_keypoints, self.keypoint_channel_configuration
        )
        return grid
    ...
    def validation_step(self, val_batch, batch_idx):
        ...
        if self.is_ap_epoch():
            ...
            log_images = (
                batch_idx == 0 and self.current_epoch > 0 and self.is_ap_epoch()
            )
    ...
    def log_and_reset_mean_ap(self, mode: str):
        mean_ap_per_threshold = torch.zeros(
            len(self.maximal_gt_keypoint_pixel_distances)
        )
        if mode == "train":
        ...
        for channel_idx, channel_name in enumerate(self.keypoint_channel_configuration):
            channel_aps = self.compute_and_log_metrics_for_channel(
        ...
        mean_ap = mean_ap_per_threshold.mean() / len(
    ...
    def update_channel_ap_metrics(
        ...
        batch_detected_channel_keypoints = [
            batch_detected_channel_keypoints[i][0] for i in range(len(gt_keypoints))
        ]
        for i, detected_channel_keypoints in enumerate(
            batch_detected_channel_keypoints
        ):
            validation_metric.update(
                detected_channel_keypoints, formatted_gt_keypoints[i]
            )
    ...
    def compute_and_log_metrics_for_channel(
        ...
        for maximal_distance, ap in ap_metrics.items():
            self.log(
                f"{training_mode}/{channel}_ap/d={float(maximal_distance):.1f}", ap
            )

        mean_ap = sum(ap_metrics.values()) / len(ap_metrics.values())
        self.log(
            f"{training_mode}/{channel}_ap/meanAP", mean_ap
    ...
    def is_ap_epoch(self) -> bool:
        ...
        is_epch = is_epch or (
            self.current_epoch > 0 and self.trainer.check_val_every_n_epoch > 1
        )
        return is_epch

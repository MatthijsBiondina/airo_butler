...
from pairo_butler.utils.tools import UGENT


class OrientationUtils:
    @staticmethod
    def plot_orientation_distribution(
        X: Tensor, distributions: Dict[str, Tensor], N_max: int = 4
    ) -> Image.Image:
        ...
        for ii in range(min(N, X.shape[0])):
            ...
            for jj, (label, logits) in enumerate(distributions.items()):
                ...
                ax_graph.plot(
                    theta,
                    P[ii].cpu().detach().numpy(),
                    label=label,
                    color=UGENT.SECONDARY_COLORS[jj],
                )
    ...
    @staticmethod
    def plot_to_wandb(
        X: Tensor, y: Tensor, t: Tensor, N: int = 3, title: str = "sample_plot"
    ) -> None:
        N = min(N, X.shape[0])
        img = OrientationUtils.plot_orientation_distribution(
            X[:N], {"y": y[:N], "t": t[:N]}, N_max=N
        )

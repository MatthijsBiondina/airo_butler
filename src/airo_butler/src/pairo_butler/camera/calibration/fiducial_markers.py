...
AIRO_DEFAULT_ARUCO_DICT: ArucoDictType = aruco.getPredefinedDictionary(
    aruco.DICT_4X4_250
)
AIRO_DEFAULT_CHARUCO_BOARD: CharucoDictType = aruco.CharucoBoard(
    (7, 5), 0.04, 0.031, AIRO_DEFAULT_ARUCO_DICT
)
...
def detect_aruco_markers(
    image: OpenCVIntImageType, dictionary: ArucoDictType
) -> Optional[ArucoMarkerDetectionResult]:
...
def refine_corner_detection(
    image: OpenCVIntImageType, corners: np.ndarray
) -> np.ndarray:
...
def get_poses_of_aruco_markers(
    ...
    if rvecs is None and tvecs is None:
        ...
        raise ValueError(
            "rvecs and tvecs should have the same shape. Do you have multiple markers with the same ID?"
        )

    # combine the rvecs and tvecs into a single pose matrix
    marker_poses_in_camera_frame = [
        SE3Container.from_rotation_vector_and_translation(
            rvec[0], tvec
        ).homogeneous_matrix
...
def detect_charuco_board(
    ...
    charuco_pose = get_pose_of_charuco_board(
        charuco_result, charuco_board, camera_matrix, dist_coeffs
    )
    return charuco_pose
...
def visualize_aruco_detections(
    ...
    image = aruco.drawDetectedMarkers(
        image, [x for x in aruco_result.corners], aruco_result.ids
    )
    return image
...
def visualize_charuco_detection(
    ...
    image = aruco.drawDetectedCornersCharuco(
        image, np.array(result.corners), np.array(result.ids), (255, 255, 0)
    )
    return image
...
if __name__ == "__main__":  # noqa: C901 - ignore complexity
    ...
    @click.command()
    @click.option(
        "--aruco_marker_size", default=0.031, help="Size of the aruco marker in meters"
    )
    @click.option(
        "--charuco_x_count",
        default=7,
    ...
    def visualize_marker_detections(
        ...
        detect_charuco = (
            charuco_x_count is not None
            and charuco_y_count is not None
            and charuco_tile_size is not None
        )
        if detect_charuco:
        ...
        while True:
            ...
            if aruco_result:
                charuco_result = detect_charuco_corners(
                    image, aruco_result, charuco_board
                )

                if charuco_result:
                    charuco_pose = get_pose_of_charuco_board(
                        charuco_result, charuco_board, intrinsics
                    )
            if aruco_result:
